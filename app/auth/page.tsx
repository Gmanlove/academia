...
export default function AuthPage() {
  ...
  useEffect(() => {
    ...
    const lockTime = localStorage.getItem('auth-lock-time')
    if (lockTime) {
      ...
      if (timePassed < lockDuration) {
        setIsLocked(true)
        setLockTimeRemaining(Math.ceil((lockDuration - timePassed) / 1000))
      } else {
  ...
  useEffect(() => {
    ...
    if (verified === 'true') {
      ...
      // Clear the verification success message after 5 seconds
      setTimeout(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isLocked) {
      setError(`Account locked. Please try again in ${Math.ceil(lockTimeRemaining / 60)} minutes.`)
      return
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (newAttempts >= 5) {
          ...
          localStorage.setItem('auth-lock-time', Date.now().toString())
            ...
            {isLocked && (
              <Alert className="mb-6 border-orange-200 bg-orange-50">
                ...
                <AlertDescription className="text-orange-700">
                  Account locked for security. Time remaining: {formatTime(lockTimeRemaining)}
                </AlertDescription>
              </Alert>
            )}
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-blue-800 space-y-1">
                      ...
                      <div><strong>Teacher:</strong> teacher / teach123</div>
                      <div><strong>Student:</strong> student / stud123</div>
                    </div>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent>
                ...
                <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center text-sm text-gray-600">
                    ...
                    <span>Protected by SSL encryption • {5 - loginAttempts} attempts remaining</span>
                  </div>
                </div>
              </CardContent>
            </Card>
